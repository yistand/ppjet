...
void plotpythia8(TString filename="FullJet_TransCharged_pythia8215_pp200hard_PionDecayOff_seed134123_170422.root", bool flagMC05 = true) {
//
...
//
	...
	TProfile *ptranptave = new TProfile("TranPtAve","ptranptave",nptbin,ptbins);
		...
		if(!flagMC05) {
			pleadntrk->Fill(jpt,leadntrk);		
			psubntrk->Fill(jpt,subntrk);		
			ptrantotntrk->Fill(jpt,maxntrk+minntrk);		

			pleadptsum->Fill(jpt,leadptsum);		
			psubptsum->Fill(jpt,subptsum);		
			ptrantotptsum->Fill(jpt,maxptsum+minptsum);		
		
			for(int i = 0; i<leadntrk; i++) {
				pleadptave->Fill(jpt,leadpt[i]);
			}
			for(int i = 0; i<subntrk; i++) {
				psubptave->Fill(jpt,subpt[i]);
			}
			for(int i = 0; i<maxntrk; i++) {
				ptranptave->Fill(jpt,maxpt[i]);
			}
			for(int i = 0; i<minntrk; i++) {
				ptranptave->Fill(jpt,minpt[i]);
			}
		}
		else {
			...
			pleadntrk->Fill(jpt,leadntrk05);		
			psubntrk->Fill(jpt,subntrk05);		
			ptrantotntrk->Fill(jpt,maxntrk05+minntrk05);		

			pleadptsum->Fill(jpt,leadptsum05);		
			psubptsum->Fill(jpt,subptsum05);		
			ptrantotptsum->Fill(jpt,maxptsum05+minptsum05);		
