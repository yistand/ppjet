...
//
//	2018.08.13	Li YI
//	plot NFweighted JPs unfolded with MB, vs noweight JPs unfolded with JP0 for Towards, Away & Transverse on the same canvas
//
//=====================================================================================================
//
//	2017.11.02	Li YI
...
void plotcompareTrigUnfold_JP0vsNF() {
	...
	const int Nunf = 2;
	//char unfoldtag[Nunf][20] = {"Charged","Charged_NFweight"};
	char unfoldtag[Nunf][20] = {"0Charged","Charged_NFweight"};
	...
	char filetag[Nunf][100] = {"MC simu trig","data-driven"};
	char filetag1[Nunf][100] = {"JP0","MB"};
	...
	for(int i = 0; i<Nvar; i++) {
		for(int j = 0; j<Nunf; j++) {
			if(j==0) {
				...
				fin[i][j] = new TFile(Form(filenameformat2,filenametag[i],unfoldtag[j],filetag1[j]));
			}
	...
	for(int i = 0; i<2; i++) {
		...
		if(savefig) {
			...
			if(strstr(filenametag[0],"TransNeutral")) c[i]->SaveAs(Form("CompareUnfoldTrigs_JP0vsNF_%s.png",outtag.Data()));
			else c[i]->SaveAs(Form("CompareUnfoldTrigs_JP0vsNF_%s.png",outtag.Data()));
		}
	}
